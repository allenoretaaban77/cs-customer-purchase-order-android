...
public class QueryBuilder {
    ...
    private static String getFieldName(Object field) {
        ...
        if(field instanceof aChecklistKey)
            fieldName = ((aChecklistKey)field).getKey();
        if(field instanceof aItemsKey)
            fieldName = ((aItemsKey)field).getKey();
        if(field instanceof aUsersKey)
            fieldName = ((aUsersKey)field).getKey();
        if(field instanceof aStaffsKey)
            fieldName = ((aStaffsKey)field).getKey();
        return fieldName;
    }
    ...
    private static String getDataType(Object field) {
        ...
        if(field instanceof aChecklistKey)
            dataType = ((aChecklistKey)field).getDataType();
        if(field instanceof aItemsKey)
            dataType = ((aItemsKey)field).getDataType();
        if(field instanceof aUsersKey)
            dataType = ((aUsersKey)field).getDataType();
        if(field instanceof aStaffsKey)
            dataType = ((aStaffsKey)field).getDataType();
        return dataType;
    }
